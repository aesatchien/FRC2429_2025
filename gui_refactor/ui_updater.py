    ...
    def update_widgets(self):
        ...
        for key, d in self.ui.widget_dict.items():
            ...
            if key not in self.keylist:
                ...
                if 'combo' in key:
                    print(f'Added {key} to keylist')

            if widget not in self.widgetlist:
                self.widgetlist.append(widget)
                # print(f'Added {widget} to widgetlist')

            # Handle all other standard widgets that rely on an NT topic 'entry'
            entry = d.get('entry')
            if entry is not None:
                if 'indicator' in key:
                    is_on = entry.getBoolean(False)
                    ...
                    value = int(entry.getDouble(0))
                    widget.display(str(value))
                elif 'monitor' in key:
                    value = entry.getDouble(0)
                    widget.set_value(value)
                # Handle the combo box as a special case because it uses multiple NT entries
                elif 'combo' in key:
                    # print(f'found {key}')
                    new_list = entry.getStringArray([])
                    ...
                    if selected:
                        selected_routine = selected.getString('')
                ...
                else:  # poses and distances
                    pass
                    # print(f'entry with no handler: {key}')
        ...
        if 'hub_targets' in self.ui.widget_dict and self.ui.widget_dict['hub_targets'].get('entry'):
        ...
        # update the pose
        if 'drive_pose' in self.ui.widget_dict and self.ui.widget_dict['drive_pose'].get('entry'):
            ...
            if 'qlabel_position_indicator' in self.ui.widget_dict and self.ui.widget_dict['qlabel_position_indicator'].get('entry'):
