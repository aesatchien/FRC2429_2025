...
class IncrementElevatorAndPivot(commands2.Command):
    ...
    def initialize(self) -> None:
        ...
        for target_name in ["elevator_height", "elbow_pivot", "shoulder_pivot"]:
            print(f"{target_name} Target Position", self.targets[target_name])
            SmartDashboard.putNumber(f"INCR {target_name} Target Position", self.targets[target_name])
        SmartDashboard.putString(f"INCR Elevator Target Position Name", self.elevator.get_target_pos())
