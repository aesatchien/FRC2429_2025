...
class GoToReefPosition(commands2.SequentialCommandGroup):
    def __init__(self, container, level: int, auto: bool = False, indent=0) -> None:
        """
        ...
        if auto is true, it will use an old l4 setpoint which worked in auto
        """
        ...
        if level != 4 and auto:
            raise ValueError("there is no auto variant for non-l4 positions. see k_positions in constants.py")
        ...
        if level == 1:
            ...
            if auto:
                self.addCommands(commands2.ParallelCommandGroup(
                    ...
                    WaitCommand(min(0, level - 2) * 0.5).andThen(MoveElevator(container=container, elevator=container.elevator, mode="specified", height=constants.k_positions[f"l{level}_auto"]["elevator"], wait_to_finish=True, indent=indent+1)).withTimeout(5),
                    MovePivot(container=container, pivot=self.container.pivot, mode="specified", angle=constants.k_positions[f"l{level}_auto"]["shoulder_pivot"], wait_to_finish=True, indent=indent+1).withTimeout(5),
                ))
