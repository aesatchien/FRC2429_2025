...
class Swerve (Subsystem):
    ...
    def get_nearest_tag(self, destination='stage'):
        ...
        if destination == 'reef':
            ...
            x_offset, y_offset = -0.10, 0.10  # subtracting translations below makes +x INTO the tag, +y LEFT of tag
            robot_offset = geo.Pose2d(geo.Translation2d(x_offset, y_offset), geo.Rotation2d(0))
            face_tag = True  # do we want to face the tag?
        elif destination == 'coral station':
            tags = [1, 2, 12, 13]
            x_offset, y_offset = -0.37, 0  # subtracting translations below makes +1 INTO the tag, +y LEFT of tag
            robot_offset = geo.Pose2d(geo.Translation2d(x_offset, y_offset), geo.Rotation2d(0))
            face_tag = True
        elif destination == 'coral side':
            tags = [3, 16]  # right one facing blue, left one facing red
            x_offset, y_offset = -1.5, 0  # subtracting translations below makes +1 INTO the tag
            robot_offset = geo.Pose2d(geo.Translation2d(x_offset, y_offset), geo.Rotation2d(0))
            face_tag = False
        else:
